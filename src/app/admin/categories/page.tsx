    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white/40 backdrop-blur-xl rounded-[40px] shadow-2xl shadow-slate-200/50 border border-white/60 p-10 flex flex-col">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto pr-2 max-h-[800px] scrollbar-thin scrollbar-thumb-slate-200">
            {categories.map((category) => (
              <div
                key={category.id}
                className="group relative rounded-[40px] bg-white border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all duration-500 overflow-hidden h-[280px]"
              >
                ...
                <div className="absolute inset-0 z-0">
                  {category.image_url ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-indigo-50 to-slate-100 flex items-center justify-center">
                      <FiPackage className="h-16 w-16 text-indigo-200 group-hover:text-indigo-300 transition-colors" />
                    </div>
                  )}
                ...
                <div className="relative z-10 h-full p-8 flex flex-col justify-end">
                  <div className="flex items-end justify-between gap-6">
                    <div className="flex-1">
                      <h3 className="text-3xl font-black text-white tracking-tight drop-shadow-lg mb-2">
                        {category.name}
                      </h3>
                      <p className="text-slate-200 text-sm font-medium line-clamp-2 max-w-sm drop-shadow-md">
                        {category.description || "Unleash the full potential of this collection with a custom description."}
                      </p>
                    </div>
                    ...
                    <div className="flex gap-3 mb-1">
                      <button
                        ...
                        className="h-14 w-14 rounded-2xl bg-white/10 backdrop-blur-md text-white border border-white/20 shadow-lg flex items-center justify-center hover:bg-white hover:text-indigo-600 transition-all duration-300 group/btn"
                      >
                        <FiSettings className="h-6 w-6 group-hover/btn:rotate-90 transition-transform duration-500" />
                      </button>
                      <button
                        onClick={() => handleDelete(category.id)}
                        className="h-14 w-14 rounded-2xl bg-white/10 backdrop-blur-md text-rose-200 border border-white/20 shadow-lg flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all duration-300"
                      >
