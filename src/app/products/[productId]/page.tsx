...
export async function generateMetadata({
  ...
  const { data: product, error: productError } = await supabase
    .from("products")
    .select("id, name, description, price, compare_at_price, stock, sku, slug, images, sizes, options, category:categories!products_category_id_fkey(name, slug)")
    .eq("status", "active")
    .eq("slug", slug)
...
export default async function SingleProductPage({
  ...
  const { data: product, error: productError } = await supabase
    .from("products")
    .select("id, name, description, price, compare_at_price, stock, sku, slug, images, sizes, options, category:category_id(name, slug)")
    .eq("slug", slug)
    .single() as { data: any | null, error: any };
